...
public class mod_moreHealthEnhanced {
	...
	private void updateConfig() {
		...
		levelRamp = config.get("general", "Level Ramp",
	...
	public void load(FMLInitializationEvent event) {
		...
		if (!RpgMode) {
			...
			try {
				...
				System.out.println(
						"There is a error in your config file. Make sure there is no extra ',' in the line for Ramp. ");
			}
		}
		if (HeartContainersAndPieces) {
			...
			GameRegistry.addRecipe(new ItemStack(heartContainer, 1),
					new Object[] { "XX", "XX", Character.valueOf('X'), heartPiece });
	...
	private void addChestLoot() {
		...
		blacksmith
				.addItem(new WeightedRandomChestContent(new ItemStack(heartContainer), 1, 1, (int) (8.0D * multiply)));
