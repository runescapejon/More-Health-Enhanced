...
public class FMLEventHandler {
			...
			if (oldArmor != currentArmor) {
				if ((currentArmor == null) && (oldArmor != null)) {
					...
					if (extraHearts > 0) {
						...
						player.addChatComponentMessage(new ChatComponentText("Removing the armor causes the extra "
								+ extraHearts + " enchanted hearts to fade away."));
					...
					int oldHealth = 2 * EnchantmentHelper
							.getEnchantmentLevel(mod_moreHealthEnhanced.heartArmorEnchantID, oldArmor);
					int newHealth = 2 * EnchantmentHelper
							.getEnchantmentLevel(mod_moreHealthEnhanced.heartArmorEnchantID, currentArmor);
					...
					if (healthChange < 0) {
						player.addChatComponentMessage(
								new ChatComponentText("Equipping the weaker new armor releases an extra " + healthChange
										+ " enchanted hearts."));
						stats.needClientSideHealthUpdate = true;
					}
				}
	...
	private void calculateHeartChange(EntityPlayer player, PlayerStats stats) {
		...
		if (levelIncreased(player, stats)) {
			while ((stats.count < mod_moreHealthEnhanced.LevelRampInt.length)
					&& (player.experienceLevel >= mod_moreHealthEnhanced.LevelRampInt[stats.count])) {
				player.addChatComponentMessage(
				...
				try {
					updatedModifier = player.getEntityAttribute(SharedMonsterAttributes.maxHealth)
							.getModifier(PlayerHandler.moreHealthID).getAmount() + 2.0D;
